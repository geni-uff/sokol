"""Materialize agenda Contacts from phone/email observables (worker copy).
...
def is_junk_observable(kind: str, value: str, display_name: str | None) -> bool:
    ...
    if name in _JUNK_TOKENS or val.lower() in _JUNK_TOKENS:
        return True
    if kind == "email":
        if "@" not in val:
            return True
...
def materialize_agenda_contacts(db, case_id: UUID | str) -> dict[str, int]:
    rows = db.execute(
        text("""
